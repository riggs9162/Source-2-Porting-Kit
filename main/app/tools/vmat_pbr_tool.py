            ...
            try:
                if self.mode == "Fake PBR":
                    ...
                    options = ExoPBROptions(
                        ...
                        alphablend=self.exo_alphablend
                    )
...
class VmatPBRTool(BaseTool):
    ...
    def setup_content(self):
        ...
        self.results_table.setHorizontalHeaderLabels([
            "Include", "VMAT", "Color", "Normal", "AO", "Rough", "Metal", "Emissive", "Notes"
        ])
    ...
    def scan(self):
        ...
        if not vmat_root:
            ...
            return
        if not texture_root:
            self.log("Texture root is required", "ERROR")
            return
        if not out_root:
    ...
    def _populate_table(self, entries: List[VmatEntry]):
        ...
        for entry in sorted(entries, key=lambda e: str(e.vmat_path)):
            ...
            warn_text = "; ".join(entry.warnings)
            warn_item = QTableWidgetItem(warn_text)
            self.results_table.setItem(row, 8, warn_item)
